...
app.get('/menu/query', (res, req)=>{
    const {search, limit} = req.query;
    let menuItem = [...menu];
    // gets the items that match the search
    if(search){
        menuItem = menuItem.filter((product)=>{
            return product.name.startsWith(search);
        })
    }
    // if a limit is given, it will only return that amount of items that match
    if(limit){
        menuItem = menuItem.slice(0, Number(limit));
    }
    // if the length is less than two, it will retirn the data
